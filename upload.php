...
$nombre_archivo = 'CORREO_'.date('Ymd_His').'_'.$id_usuario_temp;
...
if ($response_array['status'] = 'success') { ?>
	window.opener.document.getElementById("correoc").value = "<?echo $file_name_new?>";
	window.opener.document.getElementById("descarga_correo").innerHTML = "<a href='http://nnoc.vtr.cl/gvaldivia/escalamiento_critico/documentos/<?echo $file_name_new?>' target='_blank'>Descargar</a>";
...
<?}else{?>
	...
	ventana.opener = window.self;
	ventana.close();
	alert("Error al adjuntar el correo. Intentelo nuevamente por favor.");
<?} ?>
